...
class PySparkLocal:
    def __init__(self, **kwargs):
        ...
        self.conf = SparkConf().setAppName(self.appname).setMaster(
            kwargs.get("master", kwargs.get("spark.master", "local")))
    ...
    def loadFiles(self):
        ...
        cust_custapps_join_df = active_cust_df.join(other=active_cust_apps_df,
                                                    on=(active_cust_df.id == active_cust_apps_df.cust_id), how='inner')
        ...
        apps_custapps_join_df = self.spark.sql(
            "select aa.name as app_name, aa.id as app_id, aca.cust_id as cust_id, aca.active as cust_app_active from ALL_APPS aa, ALL_CUST_APPS aca where aa.id == aca.app_id and aa.active == 1")
        ...
        apps_custapps_join_df.write.partitionBy("app_name").format("avro").mode("overwrite").save(
            self.file_location + "active_apps_custapps.avro")
        print("loadFiles::Successfully written to avro file")
        avro_oms_df = self.spark.read.format("avro").load(self.file_location + "active_apps_custapps.avro").where(
            col("app_name") == "OMS").show()
...
if __name__ == '__main__':
    ...
    kw = {"spark.master": "local", "spark.name": "MySpark-CSV", "data.source.type": "file", "data.source.format": "csv",
          "data.source.location": "/FileStore/tables/",
          "data.source.files": "customers.csv,applications.csv,customer_applications.csv"}
    # configs = Properties()
